...
- AppImage manual cleanup is disabled on distributions without a supported ownership database; absent dpkg is never interpreted as proof of no owner.
- Debian archive staging permissions are asserted by the package acceptance test, including the archive root directory and the non-setuid helper.
